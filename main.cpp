...
#include<sstream>
using namespace std;
void logo();// function is used for print the logo (Satyansh)
void log();// this function is use for login to account
...
void creategraph(int s)
{
    ...
    int n1;
    ifstream red;
    red.open("minDist.txt");// open minDist.txt in read mode
    g->adj=new int*[s];
    for(i=0;i<s;i++)
    {
        g->adj[i]=new int[s];
